...
import { EIGHT, FIVE, FOUR, NINE, ONE, SEVEN, SIX, THREE, TWO } from '../../domain/helpers/constants/number.constants';
import { GetMoviesService } from './get-movies.service';
import { GET_MOVIES_SERVICE_ENDPOINTS } from './get-movies.service.configuration';
...
describe('MoviesService', () => {
  ...
  const mockGetMoviesServiceEntries = {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        {
          provide: GET_MOVIES_SERVICE_ENDPOINTS,
          useValue: mockGetMoviesServiceEntries,
        },
      ],
    });
  ...
  it('should get now playing movies', (done) => {
    const expectedUrl = `${mockGetMoviesServiceEntries.GET_NOW_PLAYING_MOVIES}?page=${ONE}`;
    const response = service.getMovies(ONE);
    const subscription = response.subscribe((data) => {
      expect(data).toBe(mockMoviesNowPlayingData);
      done();
    });

    const request = httpCtrl.expectOne(expectedUrl);
